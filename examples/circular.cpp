...
public:
  ...
  void f(float64 w, float64 k, float64 gp, float64 ge, float64& ff, float64& df)
  {
  ...
  void g(float64 x, float64 w, float64 k, float64 wc, float64 eta, float64& gg, float64& dg)
  {
    float64 ww = wc / w;
    float64 vp = eta * w / k;
    float64 a  = ww * ww;
...
public:
  /// constructor
  Circular(const int shape[3], const int nb, const float64 cc, const float64 gamma,
           const float64 delh)
