...
void PeriodicBoundary::pack(float64* buf, int& pos, T_array& array, T_shape& Lb, T_shape& Ub,
                            bool is_boundary)
{
  if (is_boundary)
    return;
...
template <class T_array, class T_shape>
void PeriodicBoundary::unpack(float64* buf, int& pos, T_array& array, T_shape& Lb, T_shape& Ub,
                              bool is_boundary)
{
  if (is_boundary)
    return;
...
PeriodicBoundary::PeriodicBoundary(const int Nz, const int Ny, const int Nx, const int Nb)
{
...
void PeriodicBoundary::set_field(Global& g, T_vector& eb, int nb)
{
  ...
  for (int dir = 0; dir < 3; dir++) {
    ...
    // send
    MPI_Isend(m_bufsnd[dir][0], pos, MPI_REAL8, neighbor[dir][0], tag[dir][0], MPI_COMM_WORLD,
              &request[0]);
    // receive
    MPI_Irecv(m_bufrcv[dir][1], m_bufnum, MPI_REAL8, neighbor[dir][1], tag[dir][0], MPI_COMM_WORLD,
              &request[3]);
    ...
    // send
    MPI_Isend(m_bufsnd[dir][1], pos, MPI_REAL8, neighbor[dir][1], tag[dir][1], MPI_COMM_WORLD,
              &request[1]);
    // receive
    MPI_Irecv(m_bufrcv[dir][0], m_bufnum, MPI_REAL8, neighbor[dir][0], tag[dir][1], MPI_COMM_WORLD,
              &request[2]);
...
void PeriodicBoundary::set_fluid(Global& g, T_vector& uf, T_vector& eb, int nb)
{
  const int Nb = (nb < 0) ? g.Nb : nb;
  int       neighbor[3][2];
  int       tag[3][2];
