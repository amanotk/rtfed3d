...
void BaseSolver::xflux_fluid_hll(float64 dt, Global& g, T_tensor& ff, T_vector& ebx, const int Nb)
{
  const float64 c = g.c;
  ...
  for (int iz = g.Lbz - Nb; iz <= g.Ubz + Nb; iz++) {
    for (int iy = g.Lby - Nb; iy <= g.Uby + Nb; iy++) {
      for (int ix = g.Lbx - 1; ix <= g.Ubx; ix++) {
        ...
        for (int k = 0; k < 10; k++) {
          ff(iz, iy, ix, 0, k) =
              (vmax * fxl[k] + vmin * fxr[k] - vmax * vmin * (ucr[k] - ucl[k])) * rv * dt;
        }
...
void BaseSolver::yflux_fluid_hll(float64 dt, Global& g, T_tensor& ff, T_vector& eby, const int Nb)
{
  const float64 c = g.c;
  ...
  for (int iz = g.Lbz - Nb; iz <= g.Ubz + Nb; iz++) {
    for (int iy = g.Lby - 1; iy <= g.Uby; iy++) {
      for (int ix = g.Lbx - Nb; ix <= g.Ubx + Nb; ix++) {
        ...
        for (int k = 0; k < 10; k++) {
          ff(iz, iy, ix, 1, k) =
              (vmax * fyl[k] + vmin * fyr[k] - vmax * vmin * (ucr[k] - ucl[k])) * rv * dt;
        }
...
void BaseSolver::zflux_fluid_hll(float64 dt, Global& g, T_tensor& ff, T_vector& ebz, const int Nb)
{
  const float64 c = g.c;
  ...
  for (int iz = g.Lbz - 1; iz <= g.Ubz; iz++) {
    for (int iy = g.Lby - Nb; iy <= g.Uby + Nb; iy++) {
      for (int ix = g.Lbx - Nb; ix <= g.Ubx + Nb; ix++) {
        ...
        for (int k = 0; k < 10; k++) {
          ff(iz, iy, ix, 2, k) =
              (vmax * fzl[k] + vmin * fzr[k] - vmax * vmin * (ucr[k] - ucl[k])) * rv * dt;
        }
    ...
    for (int iy = g.Lby - 1; iy <= g.Uby; iy++) {
      for (int ix = g.Lbx - 1; ix <= g.Ubx; ix++) {
        ...
        // y-face : Ez, Bz
        {
          ...
          feb(iz, iy, ix, 2) +=
              (0.5 * (vmax * ebl[1] + vmin * ebr[1]) + rc * vmax * vmin * (ebr[2] - ebl[2])) * rv *
              dt;
          feb(iz, iy, ix, 5) +=
              (0.5 * (vmax * ebl[3] + vmin * ebr[3]) - rc * vmax * vmin * (ebr[0] - ebl[0])) * rv *
              dt;
        }
        ...
        // z-face : Ey, By
        {
          ...
          feb(iz, iy, ix, 1) +=
              (0.5 * (vmax * ebl[4] + vmin * ebr[4]) - rc * vmax * vmin * (ebr[7] - ebl[7])) * rv *
              dt;
          feb(iz, iy, ix, 4) +=
              (0.5 * (vmax * ebl[6] + vmin * ebr[6]) + rc * vmax * vmin * (ebr[5] - ebl[5])) * rv *
              dt;
        }
      }
    }
    ...
    for (int iy = g.Lby - 1; iy <= g.Uby; iy++) {
      for (int ix = g.Lbx - 1; ix <= g.Ubx; ix++) {
        ...
        // z-face : Ex, Bx
        {
          ...
          feb(iz, iy, ix, 0) +=
              (0.5 * (vmax * ebl[0] + vmin * ebr[0]) + rc * vmax * vmin * (ebr[3] - ebl[3])) * rv *
              dt;
          feb(iz, iy, ix, 3) +=
              (0.5 * (vmax * ebl[2] + vmin * ebr[2]) - rc * vmax * vmin * (ebr[1] - ebl[1])) * rv *
              dt;
        }
        ...
        // x-face : Ez, Bz
        {
          ...
          feb(iz, iy, ix, 2) +=
              (0.5 * (vmax * ebl[5] + vmin * ebr[5]) - rc * vmax * vmin * (ebr[6] - ebl[6])) * rv *
              dt;
          feb(iz, iy, ix, 5) +=
              (0.5 * (vmax * ebl[7] + vmin * ebr[7]) + rc * vmax * vmin * (ebr[4] - ebl[4])) * rv *
              dt;
        }
      }
    }
