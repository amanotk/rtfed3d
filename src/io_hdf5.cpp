...
void HDF5IO::write_parameter(Global& g, std::string filename)
{
  ...
  // z
  hdf5util::create_dataset(file, "zig", H5T_NATIVE_DOUBLE, 1, &gshape[0], &gshape[0]);
  hdf5util::write_dataset(file, "zig", 1, &dims[0], &count[0], &loffset[0], &goffset[0],
                          g.zig.data());

  // y
  hdf5util::create_dataset(file, "yig", H5T_NATIVE_DOUBLE, 1, &gshape[1], &gshape[1]);
  hdf5util::write_dataset(file, "yig", 1, &dims[1], &count[1], &loffset[1], &goffset[1],
                          g.yig.data());

  // x
  hdf5util::create_dataset(file, "xig", H5T_NATIVE_DOUBLE, 1, &gshape[2], &gshape[2]);
  hdf5util::write_dataset(file, "xig", 1, &dims[2], &count[2], &loffset[2], &goffset[2],
                          g.xig.data());
...
void HDF5IO::write_field(Global& g, std::string filename, bool init)
{
  ...
  loffset[0] = 0;
  goffset[0] = Nwc - 1;
  hdf5util::write_dataset(root, "physt", 1, dims, count, loffset, goffset, &g.physt);
  hdf5util::write_dataset(root, "delt", 1, dims, count, loffset, goffset, &g.delt);
  ...
  dims[4]  = 6;
  count[4] = 6;
  hdf5util::write_dataset(root, "eb", 5, dims, count, loffset, goffset, g.ebc.data());

  dims[4]  = 10;
  count[4] = 10;
  hdf5util::write_dataset(root, "uf", 5, dims, count, loffset, goffset, g.uf.data());
