...
{
  const float64 tvd_rk3[3][4] = {1.0,       0.0,       1.0,       0.0,       3.0 / 4.0, 1.0 / 4.0,
                                 1.0 / 4.0, 1.0 / 4.0, 1.0 / 3.0, 2.0 / 3.0, 2.0 / 3.0, 2.0 / 3.0};
  ...
  // *** 1st step ***
  push_substep(dt, g, const_cast<float64*>(tvd_rk3[0]), g.uf, g.vf, g.ff, g.ebc, g.ueb, g.veb,
               g.feb, solver);

  // *** 2nd step ***
  push_substep(dt, g, const_cast<float64*>(tvd_rk3[1]), g.uf, g.vf, g.ff, g.ebc, g.ueb, g.veb,
               g.feb, solver);

  // *** 3rd step ***
  push_substep(dt, g, const_cast<float64*>(tvd_rk3[2]), g.uf, g.vf, g.ff, g.ebc, g.ueb, g.veb,
               g.feb, solver);
...
void RK3::push_substep(float64 dt, Global& g, float64 coeff[4], T_vector& uf, T_vector& vf,
  ...
  for (int iz = g.Lbz; iz <= g.Ubz; iz++) {
    for (int iy = g.Lby; iy <= g.Uby; iy++) {
      for (int ix = g.Lbx; ix <= g.Ubx; ix++) {
        ...
        for (int k = 0; k < 10; k++) {
          vc[k] = +coeff[0] * uf(iz, iy, ix, k) + coeff[1] * vc[k] +
                  coeff[2] * ((dtx * (ff(iz, iy, ix - 1, 0, k) - ff(iz, iy, ix, 0, k)) +
  ...
  for (int iz = g.Lbz - 1; iz <= g.Ubz; iz++) {
    for (int iy = g.Lby - 1; iy <= g.Uby; iy++) {
      for (int ix = g.Lbx - 1; ix <= g.Ubx; ix++) {
        // Ex
        veb(iz, iy, ix, 0) = +coeff[0] * ueb(iz, iy, ix, 0) + coeff[1] * veb(iz, iy, ix, 0) +
                             coeff[2] * (+cdty * (feb(iz, iy, ix, 5) - feb(iz, iy - 1, ix, 5)) -
                                         cdtz * (feb(iz, iy, ix, 4) - feb(iz - 1, iy, ix, 4)) -
                                         pidt * ff(iz, iy, ix, 0, 5));
        // Ey
        veb(iz, iy, ix, 1) = +coeff[0] * ueb(iz, iy, ix, 1) + coeff[1] * veb(iz, iy, ix, 1) +
                             coeff[2] * (+cdtz * (feb(iz, iy, ix, 3) - feb(iz - 1, iy, ix, 3)) -
                                         cdtx * (feb(iz, iy, ix, 5) - feb(iz, iy, ix - 1, 5)) -
                                         pidt * ff(iz, iy, ix, 1, 5));
        // Ez
        veb(iz, iy, ix, 2) = +coeff[0] * ueb(iz, iy, ix, 2) + coeff[1] * veb(iz, iy, ix, 2) +
                             coeff[2] * (+cdtx * (feb(iz, iy, ix, 4) - feb(iz, iy, ix - 1, 4)) -
                                         cdty * (feb(iz, iy, ix, 3) - feb(iz, iy - 1, ix, 3)) -
                                         pidt * ff(iz, iy, ix, 2, 5));
        // Bx
        veb(iz, iy, ix, 3) = +coeff[0] * ueb(iz, iy, ix, 3) + coeff[1] * veb(iz, iy, ix, 3) +
                             coeff[2] * (-cdty * (feb(iz, iy, ix, 2) - feb(iz, iy - 1, ix, 2)) +
                                         cdtz * (feb(iz, iy, ix, 1) - feb(iz - 1, iy, ix, 1)));
        // By
        veb(iz, iy, ix, 4) = +coeff[0] * ueb(iz, iy, ix, 4) + coeff[1] * veb(iz, iy, ix, 4) +
                             coeff[2] * (-cdtz * (feb(iz, iy, ix, 0) - feb(iz - 1, iy, ix, 0)) +
                                         cdtx * (feb(iz, iy, ix, 2) - feb(iz, iy, ix - 1, 2)));
        // Bz
        veb(iz, iy, ix, 5) = +coeff[0] * ueb(iz, iy, ix, 5) + coeff[1] * veb(iz, iy, ix, 5) +
                             coeff[2] * (-cdtx * (feb(iz, iy, ix, 1) - feb(iz, iy, ix - 1, 1)) +
