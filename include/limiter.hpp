...
///
/// $Id: limiter.hpp,v 6f2924be4891 2014/09/01 02:53:29 amano $
///
...
// -*- minnmod -*-
//
inline float64 minmod(const float64 a, const float64 b)
{
  return 0.5 * (sign(a) + sign(b)) * min(std::abs(a), std::abs(b));
}

inline float64 minmod(const float64 a, const float64 b, const float64 c)
{
  return 1.0 / 3.0 * (sign(a) + sign(b)) * std::abs(sign(a) + sign(c)) *
         min(std::abs(a), std::abs(b), std::abs(c));
}

inline float64 minmod(const float64 a, const float64 b, const float64 c, const float64 d)
{
  return 0.125 * (sign(a) + sign(b)) * std::abs((sign(a) + sign(c)) * (sign(a) + sign(d))) *
...
// -*- MC2 -*-
//
inline float64 mc2(const float64 a, const float64 b)
{
  return 0.5 * (sign(a) + sign(b)) * min(2 * std::abs(a), 2 * std::abs(b), 0.5 * std::abs(a + b));
}

//
// -*- MP5 -*-
//
inline void mp5(const float64 fm2, const float64 fm1, const float64 fc0, const float64 fp1,
                const float64 fp2, float64& ffp, float64& ffm)
{
...
inline float64 correct_c2p_full(const float64 fm2, const float64 fm1, const float64 fc0,
...
inline float64 correct_c2p(const float64 fm2, const float64 fm1, const float64 fc0,
                           const float64 fp1, const float64 fp2)
{
  static const float64 d2 = -1.0 / 24.0;
