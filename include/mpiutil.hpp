...
public:
  ...
  /// initialize MPI call
  static void initialize(int* argc, char*** argv, int shape[3], int domain[3])
  {
    int period[3] = {1, 1, 1};
  ...
  /// initialize MPI call
  static void initialize(int* argc, char*** argv, int shape[3], int domain[3], int period[3])
  {
    mpiutil* instance = getInstance();
    ...
    instance->m_outf   = tfm::format("%s_PE%04d.stdout", argv[0][0], instance->m_thisrank);
    ...
    instance->m_errf   = tfm::format("%s_PE%04d.stderr", argv[0][0], instance->m_thisrank);
    ...
    // set neighbors
    MPI_Cart_shift(instance->m_cart, 0, +1, &instance->m_nb_dim[0][0], &instance->m_nb_dim[0][1]);
    MPI_Cart_shift(instance->m_cart, 1, +1, &instance->m_nb_dim[1][0], &instance->m_nb_dim[1][1]);
    MPI_Cart_shift(instance->m_cart, 2, +1, &instance->m_nb_dim[2][0], &instance->m_nb_dim[2][1]);

    // coordinate
    MPI_Cart_coords(instance->m_cart, instance->m_thisrank, 3, instance->m_coord);

    // rank array
    for (int dir = 0; dir < 3; dir++) {
      int coord[3] = {instance->m_coord[0], instance->m_coord[1], instance->m_coord[2]};
      // allocate
  ...
  {
    mpiutil*    instance = getInstance();
    std::string filename = tfm::format("%s-%03d-%03d-%03d.%s", prefix, instance->m_coord[0],
                                       instance->m_coord[1], instance->m_coord[2], ext);
    return filename;
  }
  ...
  static void info(std::ostream& out)
  {
    ...
    out << tfm::format("\n"
                       ...
                       instance->m_nprocess, instance->m_thisrank, instance->m_outf.c_str(),
    ...
    for (int dir = 0; dir < 3; dir++) {
      ...
      out << tfm::format(" neighbor in dir %1d: "
                         ...
                         dir, cl[0], cl[1], cl[2], cc[0], cc[1], cc[2], cu[0], cu[1], cu[2]);
    }
    out << std::endl;
  }
};
