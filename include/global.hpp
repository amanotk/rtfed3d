...
protected:
  /// constructor
  Global(const int shape[3], const int nb, const float64 lightspeed, const float64 gamma)
      : boundary(0)
  {
    Nb = nb;
    Nz = shape[0];
...
public:
  ...
  template <int R>
  void int_c2f(T_vector& ebc, T_vector& ueb);

  /// interpolate EM-field fromc face center to cell center
  template <int R>
  void int_f2c(T_vector& ebc, T_vector& ueb);
