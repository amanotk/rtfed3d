...
public:
  ...
  virtual void flux_fluid(float64 dt, Global& g, T_vector& uf, T_vector& ebc, T_vector& ueb,
                          T_tensor& ff) = 0;

  // calculate numerical flux for field
  virtual void flux_field(float64 dt, Global& g, T_vector& ueb, T_vector& feb) = 0;

  void xflux_fluid_hll(float64 dt, Global& g, T_tensor& ff, T_vector& ebx, const int Nb);

  void yflux_fluid_hll(float64 dt, Global& g, T_tensor& ff, T_vector& eby, const int Nb);

  void zflux_fluid_hll(float64 dt, Global& g, T_tensor& ff, T_vector& ebz, const int Nb);
